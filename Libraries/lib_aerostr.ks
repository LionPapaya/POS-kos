...
function aeroturn {
    ...
    if heading_error < 20{set roll_max to 15. set pitch_max to 5.}
    if heading_error < 10{set roll_max to 7. set pitch_max to 2.}
    ...
    if (heading_error < 5 and heading_error > -5) or ship:altitude > AVES["MaxAeroturnalt"] or turn_side = "none" {
        set turn_roll to 0.
        set turn_pitch to 0.
...
function aerostr{
  set aerostr_pitch to (distance_pitch+turn_pitch).
  set aerostr_heading to turn_heading.
  if not(defined turn_roll){
    set turn_roll to 0.
  }
  set aerostr_roll to turn_roll.
  checksrt_inputs(aerostr_pitch,aerostr_heading,aerostr_roll).  
  lock targetPitch to aerostr_pitch.
  lock targetDirection to aerostr_heading.
  lock targetrole to aerostr_roll.
}
function checksrt_inputs{
  ...
  //if aerostr_pitch - pitch_for() > 5{
  //  set aerostr_pitch to pitch_for + 5.
  //}
  //if aerostr_pitch - pitch_for() < -5{
  //  set aerostr_pitch to pitch_for - 5.
  //}
...
function smooth_pitch_adjustment {
    ...
    log_status("Adjusted distance_pitch smoothly to " + distance_pitch).
}
