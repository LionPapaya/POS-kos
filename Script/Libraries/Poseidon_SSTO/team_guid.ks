function TEAM_guid_old{
    ...
    if POS_LOGGING_ENABLED { log "per ENMGT" to log_team_sim.txt. }
    IF team_internal["STEP"] = "BEF"{
    ...
    if team_internal["ENMGT"]{
        ...
        if POS_LOGGING_ENABLED { log team_internal["hac_entry"]["latlng"] to log_team_sim.txt. }
        set sim to simulate_trajectory_hed_pos(sim,team_internal["hac_entry"]["latlng"]).
        //log "aft_turn" to log_team_sim.txt.
        if POS_LOGGING_ENABLED { log team_internal["hac_entry"]["latlng"] to log_team_sim.txt. }
        if POS_LOGGING_ENABLED { log heading_between(sim["latlong"],team_internal["hac_entry"]["latlng"]) to log_team_sim.txt. }
        if POS_LOGGING_ENABLED { log "per ENMGT" to log_team_sim.txt. }
        ...
        if POS_LOGGING_ENABLED { log sim to simstate.log. }
        ...
        if POS_LOGGING_ENABLED { log sim["simtime"] to log_team_sim.txt. }
        until old_dist < dist_ or sim["altitude"] < team_internal["hac_entry"]["alt"]-100{
            ...
            if POS_LOGGING_ENABLED { log sim["simtime"]+ ",(" +sim["latlong"]:lat+ ","+sim["latlong"]:lng+"),"+sim["altitude"]+","+sim["surfvel"]:mag to log_team_sim_.txt. }
        ...
        if POS_LOGGING_ENABLED { log "aft_decent" to log_team_sim.txt. }
        if POS_LOGGING_ENABLED { log old_dist + " , " + calcdistance_m(team_internal["hac_entry"]["latlng"],sim["latlong"]) to log_team_sim.txt. }
        if POS_LOGGING_ENABLED { log team_internal["hac_entry"]["alt"] to log_team_sim.txt. }
        if POS_LOGGING_ENABLED { log team_internal["hac_entry"]["alt"]:typename to log_team_sim.txt. }
        if POS_LOGGING_ENABLED { log sim["altitude"] + " , " + (team_internal["hac_entry"]["alt"]-100) to log_team_sim.txt. }
        ...
        if sim["altitude"] < team_internal["hac_entry"]["alt"]-100 or sim["surfvel"]:mag < team_internal["hac_entry"]["vel"]{
            ...
            if team_internal["apch_mode"] = "ovh"{
                ...
                if POS_LOGGING_ENABLED { log "1"to "log.txt". }
                ...
                if POS_LOGGING_ENABLED { log "2"to "log.txt". }
                //log team_internal to "log.txt".
                //log out to "log.txt".
                //log sim to "log.txt".
                return out.
            ...
            if sim["surfvel"]:mag < team_internal["hac_entry"]["vel"]+ AVES["TEAM_v_margin"]{
                ...
                if POS_LOGGING_ENABLED { log "4"to "log.txt". }
                //log team_internal to "log.txt".
                //log out to "log.txt".
                //log sim to "log.txt".
                return out.
                ...
                if POS_LOGGING_ENABLED { log "5"to "log.txt". }
                //log team_internal to "log.txt".
                //log out to "log.txt".
                //log sim to "log.txt".
                return out.
            ...
            if POS_LOGGING_ENABLED { log "10"to "log.txt". }
            if POS_LOGGING_ENABLED { log calcdistance_m(team_internal["hac_entry"]["latlng"],sim["latlong"]) to "log.txt". }
            if POS_LOGGING_ENABLED { log sim["surfvel"]:mag to "log.txt". }
            if POS_LOGGING_ENABLED { log team_internal["hac_entry"]["vel"] to "log.txt". }
    ...
    if team_internal["step"] = "in"{
        ...
        SET OUT["team_input"]["step"] TO "ex".
        if POS_LOGGING_ENABLED { log "7"to "log.txt". }
        RETURN OUT.
    ...
    if team_internal["step"] = "ex"{
        ...
        if POS_LOGGING_ENABLED { log "8"to "log.txt". }
        RETURN OUT.
    }
    ...
    if POS_LOGGING_ENABLED { log "9"to "log.txt". }
    return out.
}
